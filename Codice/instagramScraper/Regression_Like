...
def preprocess_data(data):
    ...
    numeric_columns = ['Like', 'Followers', 'NumImmagini', 'Numero di Parole', 'Numero di Emoticon',
                       'Numero di Menzioni', 'Numero di Hashtag', 'Occorrenze del Luogo']
    ...
    X = data[['Followers', 'NumImmagini', 'Numero di Parole', 'Numero di Emoticon', 'Numero di Menzioni',
              'Numero di Hashtag', 'Occorrenze del Luogo']]
    y = data['Like']
...
def predict_likes(model, new_data):
    ...
    for i in range(len(new_data)):
        if 1 <= new_data.iloc[i]['Followers'] <= 10000:
            ...
            print("La predizione è influenzata in maniera negativa dal numero di follower troppo bassi!")
...
if __name__ == "__main__":
    ...
    new_data = pd.DataFrame({
        ...
        'NumImmagini': [3], # tic più immagini portano più like
        'Numero di Parole': [10], # tic con numero di parole più grande dà minori like
        'Numero di Emoticon': [1], # tic più emoticon portano più like
        'Numero di Menzioni': [1], # tic più menzioni portano più like
        'Numero di Hashtag': [1], # il numero di hashtag non influenza tanto il numero di like
...
if __name__ == "__main__":
    ...
    print("Suddivisione dei dati in train e test...")
    X_train, X_test, y_train, y_test = split_data(X, y)
    ...
    print("Predizione su un nuovo post...")
    num_follower = 47700

    new_data = pd.DataFrame({
        'Followers': [num_follower],
        'NumImmagini': [3],  # Più immagini portano più like
        'Numero di Parole': [10],  # Più parole portano meno like
        'Numero di Emoticon': [1],  # Più emoticon portano più like
        'Numero di Menzioni': [1],  # Più menzioni portano più like
        'Numero di Hashtag': [1],  # Gli hashtag non influenzano tanto
        'Occorrenze del Luogo': [3]  # Più occorrenze del luogo portano più like
    })

    # Predizione e stampa dei risultati
    predicted_likes = predict_likes(model, new_data)
    followers = new_data.iloc[0]['Followers']
    print(f"Numero di Followers della pagina: {followers}, Predicted Likes: {predicted_likes[0]:.2f}")
